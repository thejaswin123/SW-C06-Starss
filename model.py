...
def read_and_prepare_data(validation_mode, k=5, augment=True):
    ...
    if validation_mode == 'k_fold':
        ...
        for i in range(k):
            if augment:
                ...
                augmented_images, augmented_labels = augment_images(
                    folded_images[i], folded_labels[i])
        ...
        test_labels = []
        for i in range(k):
            train_images.append(np.concatenate(
                folded_images[:i] + folded_images[(i+1):]))
            train_labels.append(np.concatenate(
                folded_labels[:i] + folded_labels[(i+1):]))
            if augment:
                train_images[i] = np.concatenate(
                    ([train_images[i]] + folded_augmented_images[:i] + folded_augmented_images[(i + 1):]))
                train_labels[i] = np.concatenate(
                    ([train_labels[i]] + folded_augmented_labels[:i] + folded_augmented_labels[(i + 1):]))
            test_images.append(folded_images[i])
            test_labels.append(folded_labels[i])
            train_images[i], test_images[i] = standardize_data(
                train_images[i], test_images[i])
...
def augment_images(images, labels):
    ...
    for i in range(len(images)):
        ...
        image = np.reshape(
            images[i], (1, images[i].shape[0], images[i].shape[1], 1))
        label = labels[i]
        ...
        if 50 < label < 75:
            for batch in flip_generator.flow(image, batch_size=1):
                ...
                new_image = np.reshape(
                    batch[0], (batch[0].shape[0], batch[0].shape[1], 1))
...
def build_model():
    ...
    model.add(layers.Conv2D(
        32, (3, 3), activation='relu', input_shape=(50, 50, 1)))
...
def train_model(model, train_images, train_labels, test_images, test_labels, show_performance_by_epoch=False):
    ...
    performance_log = model.fit(
        ...
        callbacks=[EarlyStopping(
            monitor='val_mean_absolute_error', patience=5, restore_best_weights=True)],
        epochs=20,
        batch_size=64,
...
def performance_by_epoch(performance_log):
    ...
    performance_df = pd.DataFrame(
        columns=['epoch', 'train_or_test', 'loss_or_mae', 'value'])
    for i in range(len(train_loss)):
        new_row = {'epoch': epochs[i], 'train_or_test': 'train',
                   'loss_or_mae': 'loss', 'value': train_loss[i]}
        performance_df = performance_df.append(new_row, ignore_index=True)
        new_row = {'epoch': epochs[i], 'train_or_test': 'test',
                   'loss_or_mae': 'loss', 'value': test_loss[i]}
        performance_df = performance_df.append(new_row, ignore_index=True)
        new_row = {'epoch': epochs[i], 'train_or_test': 'train',
                   'loss_or_mae': 'mae', 'value': train_mae[i]}
        performance_df = performance_df.append(new_row, ignore_index=True)
        new_row = {'epoch': epochs[i], 'train_or_test': 'test',
                   'loss_or_mae': 'mae', 'value': test_mae[i]}
        performance_df = performance_df.append(new_row, ignore_index=True)
...
def generate_predictions(model, test_images, test_labels):
    ...
    processed_predictions = pd.DataFrame(
        columns=['prediction', 'actual', 'abs_error', 'category'])
    for i in range(len(raw_predictions)):
        ...
        processed_predictions = processed_predictions.append(
...
def show_validation_results(predictions, show_plots=True, print_error=True):
    ...
    if print_error:
        ...
        print('Root Mean Square Error: ' +
              str(round(float(rmse), 2)) + ' knots')

    if show_plots:

        categories = ['T. Depression', 'T. Storm', 'Category 1',
        ...
        plt.title(
            "Median Absolute Error in Neural Network's Predictions By Category")
        ...
        for category in categories:
            num_samples_tested = len(
                predictions.loc[predictions.category == category]['abs_error'])
            sns.distplot(
                predictions.loc[predictions.category == category]['abs_error'],
                label=category +
                ' (' + str(num_samples_tested) + ' samples tested)',
                hist=False,
                kde_kws={"shade": True})
            sns.despine()
...
def print_progress(action, progress, total):
    ...
    print('\r' + action + '... ' + str(percent_progress) +
          '% (' + str(progress) + ' of ' + str(total) + ')', end='')
...
if __name__ == "__main__":
    ...
    train_images, train_labels, test_images, test_labels = read_and_prepare_data(
        'k_fold', NUM_FOLDS, augment=AUGMENT)
    model = build_model()
    predictions = pd.DataFrame(
        columns=['prediction', 'actual', 'abs_error', 'category'])
    for i in range(NUM_FOLDS):
        print('\n\nTraining Fold ' + str(i + 1) +
              ' of ' + str(NUM_FOLDS) + '\n')
        model = train_model(
            model, train_images[i], train_labels[i], test_images[i], test_labels[i])
        kth_fold_predictions = generate_predictions(
            model, test_images[i], test_labels[i])
        predictions = predictions.append(
            kth_fold_predictions, ignore_index=True)
